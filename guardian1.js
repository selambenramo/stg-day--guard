...
client.on("guildMemberRemove", async member => {
  ...
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('STGXMAMY').setDescription(`${member},(\`\`${member.id}\`\`) üyesi, ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY "}& Witcher`).setTimestamp()).catch(); } else { member.guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Kick Atıldı!').setDescription(`${member} **(${member.id})** üyesi, ${entry.executor} (${entry.executor.id}) tarafından sunucudan sağ tık ile kicklendi! Kickleyen kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY "}& Witcher`).setTimestamp()).catch(err => {}); };
});
...
client.on("guildBanAdd", async (guild, user) => {
  ...
  cezalandir(entry.executor.id, "jail");
  guild.members.unban(user.id, "STGXMAMY").catch(console.error);
  let logKanali = client.channels.cache.get(ayarlar.logChannelID);
  if (logKanali) { logKanali.send(new MessageEmbed().setColor("#4B0082").setTitle('STGXMAMY').setDescription(`${user},(\`\`${user.id}\`\`) üyesi, ${entry.executor},(\`\`${entry.executor.id}\`\`) tarafından sunucudan sağ tık ile banlandı! Banlayan kişi jaile atıldı. `).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY "}& Witcher`).setTimestamp()).catch(); } else { guild.owner.send(new MessageEmbed().setColor("#00ffdd").setTitle('Sağ Tık Ban Atıldı!').setDescription(`${user} **(${user.id})** üyesi, ${entry.executor} **(${entry.executor.id})** tarafından sunucudan sağ tık ile banlandı! Banlayan kişi jaile atıldı.`).setFooter(`${client.users.cache.has(ayarlar.botOwner) ? client.users.cache.get(ayarlar.botOwner).tag : "STGXMAMY "}& Witcher`).setTimestamp()).catch(err => {}); };
});
...
client.on("channelDelete", async channel => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.channelGuard) return;
  cezalandir(entry.executor.id, "jail");
  await channel.clone({ reason: "GUARD(1) KANAL SİLDİ !" }).then(async kanal => {
